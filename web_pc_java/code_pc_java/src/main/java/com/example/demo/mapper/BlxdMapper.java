...
//    List<blxd> getList();
    ...
    @Select("select DISTINCT id,order_number,customer_name,insert_date,pinyin,shengchan,gongyingshang,boli_yanse,boli_shenjiagong,boli_shenjiagong,num,height,width,beizhu,shendan,case when shengchan = '正在加工' then 1 when shengchan = '配送少数' then 2 when shengchan = '加工完成' then 3 when shengchan = '完成' then 4 end as paixu from (select id,'' as dd,customer_name,insert_date,boli.order_number,pinyin,gongyingshang,boli_yanse,boli_shenjiagong,num,height,width,shengchan,beizhu,shendan,shuoming1,shuoming2 ,'' as type from boli_xiadan as boli left join (select customer_name,order_number,shendan,insert_date,guanlian,wancheng from lvkuang_xiadan ) as lvkuang on boli.order_number = lvkuang.order_number where (lvkuang.wancheng != '' and lvkuang.wancheng != '未审验' and lvkuang.wancheng != '推迟处理')) as dan order by paixu,order_number")
    List<blxd> getList();
    @Insert("insert into boli_xiadan(order_number,pinyin,boli_yanse,boli_shenjiagong,num,height,width,shengchan,beizhu,shuoming1,shuoming2,gongyingshang,guanlian) values(#{orderNumber},#{pinyin},#{boliYanse},#{boliShenjiagong},#{num},#{height},#{width},#{shengchan},#{beizhu},#{shuoming1},#{shuoming2},#{gongyingshang},#{guanlian})")
...
@Select("select DISTINCT id,insert_date,order_number,pinyin,gongyingshang,boli_yanse,boli_shenjiagong,num,height,width,shengchan,beizhu,shendan,shuoming1,shuoming2,type,case when shengchan = '正在加工' then 1 when shengchan = '配送少数' then 2 when shengchan = '加工完成' then 3 when shengchan = '完成' then 4 end as paixu from (select id,'' as dd,insert_date,boli.order_number,pinyin,gongyingshang,boli_yanse,boli_shenjiagong,num,height,width,shengchan,beizhu,shendan,shuoming1,shuoming2 ,'' as type from boli_xiadan as boli left join (select order_number,shendan,insert_date,guanlian,wancheng from lvkuang_xiadan ) as lvkuang on boli.guanlian = lvkuang.order_number where (lvkuang.wancheng != '未审验' and lvkuang.wancheng != '推迟处理') and gongyingshang=#{gongyingshang}) as dan order by paixu,order_number")
List<blxd> getListByBoli(String gongyingshang);
...
@Select("select DISTINCT id,insert_date,order_number,pinyin,gongyingshang,boli_yanse,boli_shenjiagong,num,height,width,shengchan,beizhu,shendan,shuoming1,shuoming2,type,case when shengchan = '正在加工' then 1 when shengchan = '配送少数' then 2 when shengchan = '加工完成' then 3 when shengchan = '完成' then 4 end as paixu from (select id,'' as dd,insert_date,boli.order_number,pinyin,gongyingshang,boli_yanse,boli_shenjiagong,num,height,width,shengchan,beizhu,shendan,shuoming1,shuoming2 ,'' as type from boli_xiadan as boli left join (select order_number,shendan,insert_date,guanlian,wancheng from lvkuang_xiadan ) as lvkuang on boli.guanlian = lvkuang.order_number where (lvkuang.wancheng != '未审验' and lvkuang.wancheng != '推迟处理')) as dan where pinyin like '%'+#{pinyin}+'%' and shengchan like '%'+#{shengchan}+'%' and insert_date>=#{ksxdrq} and insert_date<=#{jsxdrq} and gongyingshang = #{gongyingshang} order by paixu,order_number")
List<blxd> queryByBoli(String pinyin, String shengchan,String ksxdrq,String jsxdrq,String gongyingshang);
